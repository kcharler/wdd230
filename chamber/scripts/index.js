...
class Switch {
  constructor(switchMode) {
    ...
    this.switchBtn.addEventListener('keydown', (event) => this.handleKeydown(event));
  ...
  // Enable or disable dark mode based on the provided state
  applyDarkMode(state) {
  ...
  // Enable dark mode
  enableDarkMode() {
    this.switchBtn.setAttribute('aria-checked', 'true');
    const switchDot = this.switchBtn.querySelector('.switch span');
    switchDot.classList.add('moveRight');
